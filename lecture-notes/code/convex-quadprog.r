...
# plot the decision surface
decision.line <- function(w,b) {
	slope = -(w[1]/w[2])
	offset =  (b)/w[2]

	abline(offset,slope,lty="dotted",lwd=1,col="blue")
}
...
search <- function(df,b.seq) {
	...
	for (b in b.seq) {
		...
		if (soln$status == 0) {
			...
			# plot our incremental decision surface
			decision.line(w,b)
			if (interactive)
...
run <- function() {
	...
	decision.surface(soln$w,soln$b)
}
